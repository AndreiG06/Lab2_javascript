...
| Функция | Назначение | Возвращает | Останавливается при | Создаёт новый массив |
|---------|-----------|---------|---------|--------|
| `forEach()` | Выполнить действие для каждого | `undefined` | Никогда (до конца) | Нет |
| `map()` | Преобразовать элементы | Новый массив | Никогда (до конца) | Да |
| `filter()` | Отобрать нужные элементы | Новый массив | Никогда (до конца) | Да |
| `find()` | Найти первый элемент | Элемент или `undefined` | При совпадении | Нет |
| `some()` | Проверить "хотя бы один" | `true`/`false` | При первом `true` | Нет |
| `every()` | Проверить "все ли" | `true`/`false` | При первом `false` | Нет |
| `reduce()` | Накопить результат | Любое значение | Никогда (до конца) | Нет |
...
Все функции в этом файле реализуют основной функционал встроенных методов массивов JavaScript. Они полезны для:
- Обучения внутреннему устройству методов работы с массивами
- Понимания концепций функционального программирования
- Работы в окружении, где встроенные методы недоступны
